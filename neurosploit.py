#!/usr/bin/env python3
"""
PTTechAI - AI-Powered Penetration Testing Framework
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/pttechai.log'),
...
class Completer:
    def __init__(self, app):
        ...
        self.agent_roles = list(self.app.config.get('agent_roles', {}).keys())
        self.llm_profiles = list(self.app.config.get('llm', {}).get('profiles', {}).keys())
...
class PTTechAI:
    """Main framework class for PTTechAI"""
    ...
    def __init__(self, config_path: str = "config/config.json"):
        ...
        logger.info(f"PTTechAI initialized - Session: {self.session_id}")
    ...
    def experience_mode(self):
        ...
        print("""
        ╔═══════════════════════════════════════════════════════════╗
        ║       PTTechAI - EXPERIENCE MODE (WIZARD)                  ║
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>Generated by <strong>PTTechAI</strong> - AI-Powered Penetration Testing Framework</p>
    ...
    def execute_real_scan(self, target: str, scan_type: str = "full", agent_role: str = None, recon_context: Dict = None) -> Dict:
        ...
        """
        print(f"\n{'='*70}")
        print("    PTTechAI - Real Penetration Test Execution")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
...
"""
    ...
    def run_full_recon(self, target: str, with_ai_analysis: bool = True) -> Dict:
        ...
        """
        print(f"\n{'='*70}")
        print("    PTTechAI - FULL ADVANCED RECON")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
        print(f"[*] Session ID: {self.session_id}")
...
"""
    ...
    def run_ai_agent(
        ...
        print(f"\n{'='*70}")
        print("    PTTechAI AI OFFENSIVE SECURITY AGENT")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
        if prompt_file:
    ...
    def run_autonomous_agent(
        ...
        print(f"\n{'='*70}")
        print("   PTTechAI AUTONOMOUS AI AGENT")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
...
"""
    ...
    def interactive_mode(self):
        ...
        print("""
        ╔═══════════════════════════════════════════════════════════╗
        ║         PTTechAI - AI Offensive Security             ║
        ║                  Interactive Mode                         ║
        ╚═══════════════════════════════════════════════════════════╝
        """)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='PTTechAI - AI-Powered Penetration Testing Framework',
...
1. CLI MODE (Direct command-line):
   python pttechai.py --input "Your prompt" -cf context.json --llm-profile PROFILE

2. INTERACTIVE MODE (-i):
   python pttechai.py -i
   Then use commands: recon, analyze, scan, etc.

3. EXPERIENCE/WIZARD MODE (-e):
   python pttechai.py -e
   Guided step-by-step configuration - RECOMMENDED for beginners!

EXAMPLES:
=========
  # Step 1: Run recon to collect data
  python pttechai.py --recon example.com

  # Step 2: LLM-only analysis (no tool execution)
  python pttechai.py --input "Analyze for SQLi and XSS" -cf results/context_X.json --llm-profile claude_opus

  # Or use wizard mode
  python pttechai.py -e

  # Run full pentest scan with tools
  python pttechai.py --scan https://example.com
  ...
  python pttechai.py -i
    ...
    framework = PTTechAI(config_path=args.config)
    ...
    # Handle target with context file (AI pentest without recon)
        ...
        print("\n" + "="*70)
        print("QUICK START:")
        print("  1. Install tools:  python pttechai.py --install-tools")
        print("  2. Run scan:       python pttechai.py --scan https://target.com")
        print("  3. Interactive:    python pttechai.py -i")
        print("="*70)
