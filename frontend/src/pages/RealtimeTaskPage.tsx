...
const getQuickPrompts = (t: any) => [
  { label: t('realtimeTask.securityHeaders'), prompt: 'Analyze security headers and identify misconfigurations', icon: Shield },
  { label: t('realtimeTask.fullScan'), prompt: 'Perform a comprehensive security assessment including headers, cookies, CORS, and endpoint discovery', icon: Search },
  { label: t('realtimeTask.xssTest'), prompt: 'Test for Cross-Site Scripting (XSS) vulnerabilities in all input fields', icon: Zap },
  { label: t('realtimeTask.sqlInjection'), prompt: 'Check for SQL injection vulnerabilities in parameters and forms', icon: Terminal },
  { label: t('realtimeTask.directoryEnum'), prompt: 'Discover hidden directories, files, and endpoints using common wordlists', icon: Globe },
  { label: t('realtimeTask.techStack'), prompt: 'Detect technologies, frameworks, and versions used by this application', icon: Code },
]

/* ---------- Tool categories ---------- */
const getToolPrompts = (t: any) => [
  { tool: 'ffuf', label: 'FFUF', description: t('realtimeTask.ffuf'), icon: Zap },
  { tool: 'feroxbuster', label: 'Feroxbuster', description: t('realtimeTask.feroxbuster'), icon: Search },
  { tool: 'nuclei', label: 'Nuclei', description: t('realtimeTask.nuclei'), icon: AlertCircle },
  { tool: 'nmap', label: 'Nmap', description: t('realtimeTask.nmap'), icon: Globe },
  { tool: 'nikto', label: 'Nikto', description: t('realtimeTask.nikto'), icon: Shield },
  { tool: 'httpx', label: 'HTTPX', description: t('realtimeTask.httpx'), icon: ExternalLink },
]
...
export default function RealtimeTaskPage() {
  ...
  const checkLlmStatus = useCallback(async () => {
    try {
      ...
      setLlmStatus({
        available: false,
        provider: null,
        error: t('realtimeTask.failedToConnectBackend')
      })
    }
  }, [])
  ...
  const createSession = useCallback(async () => {
    ...
    try {
      ...
      setNewTarget('')
      setNewSessionName('')
      addToast(t('realtimeTask.sessionCreated'), 'success')
    } catch (err: unknown) {
  ...
  }, [newTarget, newSessionName, addToast, loadSessions, t])
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      const errObj = err as { response?: { data?: { detail?: string } } }
      const msg = errObj.response?.data?.detail || t('realtimeTask.failedToLoadSession')
      setError(msg)
    }
  }, [t])
  ...
  const sendMessage = useCallback(async (prompt?: string) => {
    ...
    try {
      ...
      if (result.findings && result.findings.length > (activeSession.findings?.length || 0)) {
        addToast(t('realtimeTask.newFindingsDiscovered'), 'info')
      }
  ...
  }, [message, activeSession, addToast, t])
  ...
  const executeTool = useCallback(async (toolId: string) => {
    ...
    addToast(t('realtimeTask.runningTool', { tool: toolId }), 'info')
    ...
    try {
      ...
      addToast(t('realtimeTask.toolCompleted', { tool: toolId }), 'success')
    } catch (err: unknown) {
      const errObj = err as { response?: { data?: { detail?: string } }; message?: string }
  ...
  }, [activeSession, addToast, loadSession, t])

  const deleteSession = useCallback(async (sessionId: string) => {
    if (!confirm(t('realtimeTask.deleteConfirm'))) return
    ...
    try {
      ...
      await loadSessions()
      addToast(t('realtimeTask.sessionDeleted'), 'success')
    } catch (err) {
      console.error('Failed to delete session:', err)
      addToast(t('realtimeTask.failedToDeleteSession'), 'error')
    }
  }, [activeSession, loadSessions, addToast, t])
  ...
  const downloadReportHtml = useCallback(async () => {
    ...
    try {
      ...
      addToast(t('realtimeTask.htmlReportGenerated'), 'success')
    } catch (err) {
      console.error('Failed to generate HTML report:', err)
      addToast(t('realtimeTask.failedToGenerateHtmlReport'), 'error')
    } finally {
      setGeneratingReport(false)
    }
  }, [activeSession, addToast, t])
  ...
  const downloadReportJson = useCallback(async () => {
    ...
    try {
      ...
      addToast(t('realtimeTask.jsonReportDownloaded'), 'success')
    } catch (err) {
      console.error('Failed to generate report:', err)
      addToast(t('realtimeTask.failedToGenerateJsonReport'), 'error')
    }
  }, [activeSession, addToast, t])
            ...
            <div className="flex items-center gap-2 mb-2 text-xs text-dark-400">
              ...
              {msg.metadata?.new_findings && msg.metadata.new_findings > 0 && (
                <span className="bg-orange-500/20 text-orange-400 px-1.5 py-0.5 rounded text-[10px]">
                  +{msg.metadata.new_findings} {t('realtimeTask.findings')}
                </span>
              )}
            </div>
          )}
            ...
            >
              {isExpanded ? (
                <>
                  ...
                  {t('realtimeTask.showLess')}
                </>
              ) : (
                <>
                  <ChevronRight className="w-3 h-3" />
                  {t('realtimeTask.showMore', { count: Math.ceil((msg.content.length - MESSAGE_MAX_LENGTH) / 100) * 100 })}
                </>
              )}
            </button>
          )}
      ...
      <div className="space-y-6" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
        ...
        >
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              {t('realtimeTask.title')}
            </h2>
            <p className="text-dark-400 mt-1">
              {t('realtimeTask.subtitle')}
            </p>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {toolsStatus && (
              <div
                ...
                title={toolsStatus.available ? t('realtimeTask.toolsReady') : t('realtimeTask.noDocker')}
                style={{ animation: 'fadeSlideIn 0.5s ease-out' }}
              >
                <Terminal className="w-3.5 h-3.5" />
                <span>{toolsStatus.available ? t('realtimeTask.toolsReady') : t('realtimeTask.noDocker')}</span>
              </div>
            )}
            <Button onClick={() => setShowNewSession(true)}>
              <Plus className="w-4 h-4 mr-2" />
              {t('realtimeTask.newSession')}
            </Button>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            ...
            <Card className="w-full max-w-md mx-4 shadow-2xl">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                ...
                {t('realtimeTask.createSession')}
              </h3>

              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-dark-300 mb-1.5 font-medium">{t('realtimeTask.targetUrl')}</label>
                  <input
                    type="text"
                    value={newTarget}
                    onChange={(e) => setNewTarget(e.target.value)}
                    placeholder={t('realtimeTask.targetPlaceholder')}
                ...
                <div>
                  <label className="block text-sm text-dark-300 mb-1.5 font-medium">{t('realtimeTask.sessionName')}</label>
                  <input
                    type="text"
                    value={newSessionName}
                    onChange={(e) => setNewSessionName(e.target.value)}
                    placeholder={t('realtimeTask.sessionNamePlaceholder')}
                    className="w-full bg-dark-800 border border-dark-600 rounded-lg px-4 py-2.5 text-white placeholder-dark-400 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-all"
                  />
                </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            ...
            <Card className="w-full max-w-2xl mx-4 shadow-2xl">
              ...
              <p className="text-dark-400 text-sm mb-4">
                {t('realtimeTask.target', { target: activeSession.target })}
              </p>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {getToolPrompts(t).map((tool, idx) => {
              ...
              {!toolsStatus?.available && (
                <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-yellow-400 text-sm flex items-center gap-2">
                  ...
                  {t('realtimeTask.dockerNotAvailable')}
                </div>
              )}
            </Card>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1" style={{ animation: 'fadeSlideIn 0.35s ease-out' }}>
            <Card title={t('realtimeTask.sessions')} className="h-fit">
                ...
                {sessions.length === 0 ? (
                  <div className="text-center py-8" style={{ animation: 'fadeSlideIn 0.4s ease-out' }}>
                    ...
                    <p className="text-dark-400 text-sm">
                      {t('realtimeTask.noSessions')}
                    </p>
                    <p className="text-dark-500 text-xs mt-1">
                      {t('realtimeTask.createOne')}
                    </p>
                  </div>
                ) : (
                  sessions.map((s, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-2 text-xs">
                        <span className="text-dark-400">{t('realtimeTask.messages', { count: s.messages_count })}</span>
                        {s.findings_count > 0 && (
                          <span className="bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full font-medium">
                            {t('realtimeTask.findings', { count: s.findings_count })}
                          </span>
                        )}
                      </div>
                    </div>
            ...
            {activeSession ? (
                ...
                <div className="flex-1 overflow-y-auto py-4 space-y-4 scroll-smooth">
                  {activeSession.messages.length === 0 ? (
                    <div className="text-center py-12" style={{ animation: 'fadeSlideIn 0.4s ease-out' }}>
                      ...
                      <p className="text-dark-300 font-medium mb-1">{t('realtimeTask.noMessages')}</p>
                      <p className="text-dark-500 text-sm max-w-xs mx-auto">
                        {t('realtimeTask.startTesting')}
                      </p>
                    </div>
                  ) : (
                  ...
                  {(isSending || executingTool) && (
                    <div className="flex justify-start" style={{ animation: 'messagePop 0.3s ease-out' }}>
                      ...
                      >
                        ...
                        <span>{executingTool ? t('realtimeTask.runningTool', { tool: executingTool }) : t('realtimeTask.analyzing')}</span>
                      </div>
                    </div>
                  )}
              ...
              <Card className="flex flex-col items-center justify-center h-[calc(100vh-220px)]">
                <div className="text-center" style={{ animation: 'fadeSlideIn 0.4s ease-out' }}>
                  ...
                  <h3 className="text-white text-lg font-medium mb-2">{t('realtimeTask.noSessionSelected')}</h3>
                  <p className="text-dark-400 text-center mb-6 max-w-sm">
                    {t('realtimeTask.selectOrCreate')}
                  </p>
                  <Button onClick={() => setShowNewSession(true)} className="mx-auto">
                    <Plus className="w-4 h-4 mr-2" />
                    {t('realtimeTask.createNewSession')}
                  </Button>
                </div>
              </Card>
            )}
          ...
          <div className="lg:col-span-1 space-y-4" style={{ animation: 'fadeSlideIn 0.45s ease-out' }}>
            ...
            <Card
              title={
                <div className="flex items-center gap-2">
                  ...
                  <span>{t('realtimeTask.findingsHeader')}</span>
                  {sortedFindings.length > 0 && (
            ...
            >
              {sortedFindings.length === 0 ? (
                <div className="text-center py-8" style={{ animation: 'fadeSlideIn 0.4s ease-out' }}>
                  ...
                  <p className="text-dark-400 text-sm">
                    {t('realtimeTask.noFindings')}
                  </p>
                  <p className="text-dark-500 text-xs mt-1">
                    {t('realtimeTask.sendInstructionToDiscover')}
                  </p>
                </div>
              ) : (
                    ...
                    >
                      ...
                      {expandedFindings.has(i) && (
                        ...
                        >
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                              {finding.cvss_score && (
                                <div className="bg-dark-800 px-2 py-1 rounded text-xs">
                                  <span className="text-dark-500">{t('realtimeTask.cvss')}:</span>{' '}
                              ...
                              {finding.cwe_id && (
                                <div className="bg-dark-800 px-2 py-1 rounded text-xs">
                                  <span className="text-dark-500">{t('realtimeTask.cwe')}:</span>{' '}
                                  <span className="text-blue-400">{finding.cwe_id}</span>
                                </div>
                              )}
                              {finding.owasp && (
                                <div className="bg-dark-800 px-2 py-1 rounded text-xs">
                                  <span className="text-dark-500">{t('realtimeTask.owasp')}:</span>{' '}
                          ...
                          <div className="mt-3">
                            <p className="text-[10px] text-dark-500 uppercase tracking-wider font-medium">{t('realtimeTask.type')}</p>
                            <p className="text-sm text-dark-300 break-words">{finding.vulnerability_type}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-dark-500 uppercase tracking-wider font-medium">{t('realtimeTask.description')}</p>
                            <p className="text-sm text-dark-300 break-words">{finding.description}</p>
                          </div>
                          {finding.evidence && (
                            <div>
                              <p className="text-[10px] text-dark-500 uppercase tracking-wider font-medium">{t('realtimeTask.evidence')}</p>
                          ...
                          <div>
                            <p className="text-[10px] text-dark-500 uppercase tracking-wider font-medium">{t('realtimeTask.remediation')}</p>
            ...
            {activeSession && activeSession.recon_data?.technologies && activeSession.recon_data.technologies.length > 0 && (
              <Card title={t('realtimeTask.technologies')} className="!p-4">
