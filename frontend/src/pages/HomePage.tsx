...
function DonutChart({ data }: { data: Array<{ name: string; value: number; color: string }> }) {
  ...
  if (filtered.length === 0) return <p className="text-dark-500 text-center py-8 text-sm">{t('dashboard.noData')}</p>
...
export default function HomePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (prevFindingsCountRef.current >= 0 && totalFindings > prevFindingsCountRef.current) {
        ...
        addToast(`${diff}${t('dashboard.newFinding')}`, 'warning')
      }
      ...
      if (prevRunningCountRef.current > 0 && runningCount < prevRunningCountRef.current) {
        addToast(t('dashboard.scanCompleted'), 'success')
      }
      ...
      if (consecutiveErrorsRef.current >= 3) {
        ...
        addToast(t('dashboard.connectionRestored'), 'success')
      }
  ...
  const severityChartData = useMemo(() => [
    { name: t('severity.critical'), value: stats?.vulnerabilities.critical || 0, color: SEVERITY_CHART_COLORS.critical },
    { name: t('severity.high'), value: stats?.vulnerabilities.high || 0, color: SEVERITY_CHART_COLORS.high },
    { name: t('severity.medium'), value: stats?.vulnerabilities.medium || 0, color: SEVERITY_CHART_COLORS.medium },
    { name: t('severity.low'), value: stats?.vulnerabilities.low || 0, color: SEVERITY_CHART_COLORS.low },
    { name: t('severity.info'), value: stats?.vulnerabilities.info || 0, color: SEVERITY_CHART_COLORS.info },
  ], [stats, t])

  const scanChartData = useMemo(() => [
    { name: t('common.running'), value: stats?.scans.running || 0, color: STATUS_CHART_COLORS.running },
    { name: t('common.completed'), value: stats?.scans.completed || 0, color: STATUS_CHART_COLORS.completed },
    { name: t('common.stopped'), value: stats?.scans.stopped || 0, color: STATUS_CHART_COLORS.stopped },
    { name: t('common.failed'), value: stats?.scans.failed || 0, color: STATUS_CHART_COLORS.failed },
    { name: t('common.pending'), value: stats?.scans.pending || 0, color: STATUS_CHART_COLORS.pending },
  ], [stats, t])
  ...
  const statCards = useMemo(() => [
    { label: t('dashboard.totalScans'), value: stats?.scans.total || 0, icon: Activity, color: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
    { label: t('dashboard.runningShort'), value: stats?.scans.running || 0, icon: Play, color: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/20' },
    { label: t('dashboard.completedShort'), value: stats?.scans.completed || 0, icon: CheckCircle, color: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20' },
    { label: t('dashboard.totalVulnsShort'), value: stats?.vulnerabilities.total || 0, icon: Bug, color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20' },
    { label: t('dashboard.criticalShort'), value: stats?.vulnerabilities.critical || 0, icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-600/10', border: 'border-red-600/20' },
    { label: t('dashboard.highShort'), value: stats?.vulnerabilities.high || 0, icon: Shield, color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/20' },
  ], [stats, t])
    ...
    <div className="space-y-6">
      ...
      {connectionLost && (
        ...
        >
          ...
          <span className="text-sm text-yellow-300">{t('dashboard.connectionIssues')}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {([
          { label: t('dashboard.autoPentest'), icon: Zap, to: '/auto', color: 'text-green-400', bg: 'bg-green-500/10 hover:bg-green-500/20', border: 'border-green-500/20 hover:border-green-500/40', desc: t('dashboard.streamAiTesting') },
          { label: t('dashboard.fullIaTesting'), icon: Shield, to: '/full-ia', color: 'text-red-400', bg: 'bg-red-500/10 hover:bg-red-500/20', border: 'border-red-500/20 hover:border-red-500/40', desc: t('dashboard.vulnTypes') },
          { label: t('sidebar.vulnLab'), icon: FlaskConical, to: '/vuln-lab', color: 'text-purple-400', bg: 'bg-purple-500/10 hover:bg-purple-500/20', border: 'border-purple-500/20 hover:border-purple-500/40', desc: t('dashboard.perTypeChallenges') },
          { label: t('sidebar.terminalAgent'), icon: Terminal, to: '/terminal', color: 'text-cyan-400', bg: 'bg-cyan-500/10 hover:bg-cyan-500/20', border: 'border-cyan-500/20 hover:border-cyan-500/40', desc: t('dashboard.aiChatCommands') },
        ] as const).map(action => (
          <Link
            key={action.to}
            to={action.to}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title={t('dashboard.vulnerabilitySeverity')}>
          <DonutChart data={severityChartData} />
        </Card>
        <Card title={t('dashboard.scanStatus')}>
          ...
          <div className="space-y-2">
            {recentScans.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-dark-400 text-sm">{t('dashboard.noScansYet')}</p>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-white truncate group-hover:text-primary-400 transition-colors">
                      {scan.name || t('dashboard.unnamedScan')}
                    </p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-dark-500">{relativeTime(scan.created_at, t)}</span>
                      {scan.status === 'running' && (
                  ...
                  <div className="flex items-center gap-2 ml-2">
                    ...
                    {scan.total_vulnerabilities > 0 && (
                      <span className="text-xs text-dark-400 tabular-nums">
                        {scan.total_vulnerabilities} {t('dashboard.vulns')}
                      </span>
                    )}
                  </div>
                </Link>
          ...
          <div className="space-y-2">
            {recentVulnerabilities.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-dark-400 text-sm">{t('dashboard.noVulnsFound')}</p>
              </div>
            ) : (
                  ...
                  <div className="flex items-center gap-1.5 ml-2">
                    ...
                    {vuln.validation_status === 'false_positive' && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-dark-600 text-dark-400">
                        {t('dashboard.falsePositiveShort')}
                      </span>
                    )}
        ...
        action={
          <div className="flex items-center gap-1 flex-wrap">
            {(['all', 'scan', 'vulnerability', 'agent_task', 'report'] as const).map(f => (
              ...
              >
                {f === 'all' ? t('dashboard.all')
                  : f === 'agent_task' ? t('dashboard.tasks')
                  : f === 'vulnerability' ? t('dashboard.vulnsShort')
                  : f === 'scan' ? t('dashboard.scansShort')
                  : f === 'report' ? t('dashboard.reportsShort')
                  : f}
              </button>
            ))}
          </div>
      ...
      >
        <div className="space-y-1.5 max-h-[400px] overflow-auto">
          {filteredActivity.length === 0 ? (
            <p className="text-dark-400 text-center py-8 text-sm">{t('dashboard.noRecentActivity')}</p>
          ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-dark-500 uppercase font-medium">
                      {activity.type === 'agent_task' ? t('dashboard.tasks')
                        : activity.type === 'vulnerability' ? t('dashboard.vulnsShort')
                        : activity.type === 'scan' ? t('dashboard.scansShort')
                        : activity.type === 'report' ? t('dashboard.reportsShort')
                        : activity.type}
                    </span>
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  ...
                  {activity.status && !activity.severity && (
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
                      ...
                      {activity.status === 'completed' ? t('common.completed')
                        : activity.status === 'running' ? t('common.running')
                        : activity.status === 'failed' ? t('common.failed')
                        : activity.status === 'stopped' ? t('common.stopped')
                        : activity.status}
                    </span>
                  )}
                  <span className="text-[10px] text-dark-500">{relativeTime(activity.timestamp, t)}</span>
                </div>
              </Link>
            ))
          )}
