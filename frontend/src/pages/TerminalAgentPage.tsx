...
const getTemplates = (t: any): SessionTemplate[] => [
  {
    id: 'network_scanner',
    name: t('terminal.templates.networkScanner'),
    description: t('terminal.templates.networkScannerDesc'),
    icon: 'globe',
    accent: 'blue',
  },
  {
    id: 'lateral_movement',
    name: t('terminal.templates.lateralMovement'),
    description: t('terminal.templates.lateralMovementDesc'),
    icon: 'arrows',
    accent: 'orange',
  },
  {
    id: 'privilege_escalation',
    name: t('terminal.templates.privilegeEscalation'),
    description: t('terminal.templates.privilegeEscalationDesc'),
    icon: 'shield',
    accent: 'red',
  },
  {
    id: 'vpn_recon',
    name: t('terminal.templates.vpnRecon'),
    description: t('terminal.templates.vpnReconDesc'),
    icon: 'wifi',
    accent: 'green',
  },
]
...
export default function TerminalAgentPage() {
  ...
  const loadSession = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      addToast(t('terminal.failedToLoadSession'), 'error')
    } finally {
      setLoading(false)
    }
  }, [addToast, t])
  ...
  const createSession = async () => {
    ...
    try {
      ...
      addToast(t('terminal.sessionCreated'), 'success')
    } catch (err) {
      console.error('Failed to create session:', err)
      addToast(t('terminal.failedToCreateSession'), 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const deleteSession = async (sessionId: string) => {
    if (!confirm(t('terminal.deleteConfirm'))) return
    try {
      ...
      await loadSessions()
      addToast(t('terminal.sessionDeleted'), 'info')
    } catch (err) {
      console.error('Failed to delete session:', err)
      addToast(t('terminal.failedToDeleteSession'), 'error')
    }
  }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errorMsg: TerminalMessage = {
        role: 'system',
        content: `Error: ${error?.response?.data?.detail || error?.message || t('terminal.failedToSendMessage')}`,
        timestamp: new Date().toISOString(),
      }
      setSessionData(prev =>
        prev ? { ...prev, messages: [...prev.messages, errorMsg] } : prev,
      )
      addToast(t('terminal.failedToSendMessage'), 'error')
    } finally {
  ...
  const executeCommand = useCallback(async (cmd?: string) => {
    ...
    try {
      ...
      const errorMsg: TerminalMessage = {
        role: 'tool',
        content: error?.response?.data?.detail || error?.message || t('terminal.commandExecutionFailed'),
        timestamp: new Date().toISOString(),
        exit_code: -1,
      }
      setSessionData(prev =>
      ...
      addToast(t('terminal.commandExecutionFailed'), 'error')
    } finally {
  ...
  const handleVpnUploadAndConnect = async () => {
    ...
    try {
      ...
      setVpnUsername('')
      setVpnPassword('')
      addToast(t('terminal.vpnConnected'), 'success')
    } catch (err: unknown) {
      const error = err as { response?: { data?: { detail?: string } }; message?: string }
  ...
  const handleVpnDisconnect = async () => {
    ...
    try {
      ...
      addToast(t('terminal.vpnDisconnected'), 'info')
    } catch {
      addToast(t('terminal.failedToDisconnectVpn'), 'error')
    }
  }
          ...
          <div key={index} className="flex justify-start" style={{ animation: `fadeSlideIn 0.3s ease-out ${animDelay} both` }}>
            <div className="max-w-[85%] rounded-xl px-4 py-3 bg-dark-800/80 text-dark-200 shadow-lg border border-dark-700/50 hover:shadow-xl transition-shadow">
              <div className="flex items-center gap-2 mb-2 text-xs text-dark-400">
                ...
                <span className="font-medium">{t('terminal.terminalAgent')}</span>
              </div>
              <div className="whitespace-pre-wrap text-sm leading-relaxed break-words">
                {msg.content}
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
            {sessions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
                ...
                <p className="text-dark-500 text-sm font-medium mb-1">{t('terminal.noSessions')}</p>
                <p className="text-dark-600 text-xs">{t('terminal.createSessionToStart')}</p>
              </div>
            ) : (
              sessions.map((s, idx) => (
                <div
        ...
        <div className="flex-1 flex flex-col bg-dark-900 min-w-0">
          {activeSession && sessionData ? (
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-white font-medium text-sm truncate">
                      {sessionData.name || t('terminal.terminalSession')}
                    </h3>
                ...
                {sendingMessage && (
                  <div className="flex justify-start" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
                    <div className="flex items-center gap-2 px-4 py-3 bg-dark-800 border border-dark-700/50 rounded-xl text-dark-400 text-sm shadow-md">
                      ...
                      <span>{t('terminal.agentThinking')}</span>
                ...
                {executingCommand && (
                  <div className="flex justify-start" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
                    <div className="flex items-center gap-2 px-4 py-3 bg-dark-950 border border-green-500/20 rounded-xl text-green-400 text-sm font-mono shadow-md">
                      ...
                      <span>{t('terminal.executingCommand')}</span>
                      <span style={{ animation: 'terminalBlink 1.5s ease-in-out infinite' }}>...</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-2 items-center">
                  <div className="flex-1 flex items-center bg-dark-950 border border-dark-700 rounded-lg overflow-hidden focus-within:border-green-500/50 focus-within:ring-1 focus-within:ring-green-500/30 transition-all">
                    ...
                    <input
                      ...
                      placeholder={t('terminal.enterCommandPlaceholder')}
                      disabled={sendingMessage || executingCommand}
                  ...
                  <div className="flex items-center gap-2 pl-2 border-l border-dark-700">
                    ...
                    <span className={`text-xs font-medium whitespace-nowrap ${useSandbox ? 'text-green-400' : 'text-red-400'}`}>
                      {useSandbox ? t('terminal.sandbox') : t('terminal.direct')}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
              ...
              <h3 className="text-white text-lg font-semibold mb-2">{t('terminal.title')}</h3>
            ...
            {vpnStatus?.connected ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-400">{t('terminal.connected')}</p>
                  ...
                  {vpnStatus.latency_ms != null && (
                    <p className="text-xs text-dark-500">{vpnStatus.latency_ms}ms {t('terminal.latency')}</p>
                  )}
                </div>
                <button
              ...
              <>
                <div className="flex items-center gap-3 p-3 rounded-lg border bg-dark-900 border-dark-700 mb-3">
                  ...
                  <span className="text-dark-400 text-sm">
                    {activeSession ? t('terminal.notConnected') : t('terminal.noSession')}
                  </span>
                </div>
                  ...
                  <div className="space-y-2" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
                    ...
                    {vpnFile && (
                      <div className="space-y-2" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
                        <input
                          type="text"
                          placeholder={t('terminal.vpnUsername')}
                          value={vpnUsername}
                        ...
                        <input
                          type="password"
                          placeholder={t('terminal.vpnPassword')}
                          value={vpnPassword}
                        ...
                        >
                          {vpnUploading || vpnConnecting ? (
                            <>
                              ...
                              {vpnUploading ? t('terminal.uploading') : t('terminal.connecting')}
                            </>
                          ) : (
                            <>
                              <Wifi className="w-3.5 h-3.5" />
                              {t('terminal.connectVpn')}
                            </>
                          )}
                        </button>
                      </div>
            ...
            {hasExploitationSteps && sessionData ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-8" style={{ animation: 'fadeSlideIn 0.3s ease-out' }}>
                ...
                <p className="text-dark-500 text-xs leading-relaxed max-w-[180px]">
                  {activeSession
                    ? t('terminal.noExploitationSteps')
                    : t('terminal.selectSessionToView')}
                </p>
              </div>
            )}
          </div>
              ...
              <div className="px-6 pb-6 space-y-5">
                ...
                <div style={{ animation: 'fadeSlideIn 0.3s ease-out 0.2s both' }}>
                  <label className="block text-sm font-medium text-dark-300 mb-1.5">
                    {t('terminal.target')} <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="text"
                    value={newSessionTarget}
                    onChange={e => setNewSessionTarget(e.target.value)}
                    placeholder={t('terminal.targetPlaceholder')}
                    autoFocus
                ...
                <div style={{ animation: 'fadeSlideIn 0.3s ease-out 0.25s both' }}>
                  <label className="block text-sm font-medium text-dark-300 mb-1.5">
                    {t('terminal.sessionName')} <span className="text-dark-600">({t('common.optional')})</span>
                  </label>
                  <input
                    type="text"
