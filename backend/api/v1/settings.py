"""
PTTechAI v3 - Settings API Endpoints
"""
import os
import re
import time
...
async def clear_database(
    ...
    """Clear all business data from the database (admin only, preserves users/permissions)"""
    # Only admin can clear database
    if current_user.role != Role.ADMIN:
        raise HTTPException(status_code=403, detail="Only admin can clear database")

    # Require explicit confirmation
    if not request.confirm:
        raise HTTPException(status_code=400, detail="confirm=true required to clear database")

    try:
        # Delete in correct order to respect foreign key constraints
        # 1. Child tables with foreign keys first
        await db.execute(delete(AgentTask))
        await db.execute(delete(VulnLabChallenge))
        ...
        return {
            ...
            "status": "success",
            "cleared_tables": [
                "agent_tasks", "vuln_lab_challenges", "vulnerability_tests",
                "vulnerabilities", "endpoints", "reports", "targets", "scans"
            ],
            "preserved_tables": [
                "users", "api_keys", "permissions", "role_permissions", "prompts"
            ]
        }
    except Exception as e:
        await db.rollback()
