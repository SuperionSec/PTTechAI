"""
PTTechAI v3 - Reports API Endpoints
"""
...
async def list_reports(
    ...
    if current_user.role != Role.ADMIN:
        query = query.join(Scan, Report.scan_id == Scan.id).where(Scan.user_id == current_user.id)
...
async def generate_report(
    ...
    if current_user.role != Role.ADMIN and scan.user_id != current_user.id:
...
async def view_report(
    ...
    """View report in browser (HTML) - optional auth for window.open compatibility"""
    result = await db.execute(select(Report).where(Report.id == report_id))
    report = result.scalar_one_or_none()
...
async def download_report(
    ...
    """Download report in specified format - optional auth for window.open compatibility"""
    result = await db.execute(select(Report).where(Report.id == report_id))
    report = result.scalar_one_or_none()
    ...
    # If user is authenticated, check ownership
    if current_user and current_user.role != Role.ADMIN and scan.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Access denied")
...
async def download_report_zip(
    ...
    """Download report as ZIP with screenshots included - optional auth for window.open compatibility"""
    import zipfile
    import tempfile
    import hashlib
