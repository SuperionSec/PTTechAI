...
async def _run_migrations(conn):
    ...
    try:
        ...
        # Check and add user_id column to scans table
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to scans table...")
            await conn.execute(text("ALTER TABLE scans ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to targets table
        result = await conn.execute(text("PRAGMA table_info(targets)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to targets table...")
            await conn.execute(text("ALTER TABLE targets ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to prompts table
        result = await conn.execute(text("PRAGMA table_info(prompts)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to prompts table...")
            await conn.execute(text("ALTER TABLE prompts ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to endpoints table
        result = await conn.execute(text("PRAGMA table_info(endpoints)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to endpoints table...")
            await conn.execute(text("ALTER TABLE endpoints ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to vulnerabilities table
        result = await conn.execute(text("PRAGMA table_info(vulnerabilities)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to vulnerabilities table...")
            await conn.execute(text("ALTER TABLE vulnerabilities ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to vulnerability_tests table
        result = await conn.execute(text("PRAGMA table_info(vulnerability_tests)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to vulnerability_tests table...")
            await conn.execute(text("ALTER TABLE vulnerability_tests ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to reports table
        result = await conn.execute(text("PRAGMA table_info(reports)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to reports table...")
            await conn.execute(text("ALTER TABLE reports ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to agent_tasks table
        result = await conn.execute(text("PRAGMA table_info(agent_tasks)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to agent_tasks table...")
            await conn.execute(text("ALTER TABLE agent_tasks ADD COLUMN user_id VARCHAR(36)"))
        
        # Check and add user_id column to vuln_lab_challenges table
        result = await conn.execute(text("PRAGMA table_info(vuln_lab_challenges)"))
        columns = [row[1] for row in result.fetchall()]
        if "user_id" not in columns:
            logger.info("Adding 'user_id' column to vuln_lab_challenges table...")
            await conn.execute(text("ALTER TABLE vuln_lab_challenges ADD COLUMN user_id VARCHAR(36)"))
...
async def init_db():
    ...
    async with engine.begin() as conn:
        if _is_sqlite():
            ...
            logger.info("SQLite WAL mode enabled (concurrent reads + fast writes)")
