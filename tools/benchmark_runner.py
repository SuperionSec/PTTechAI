#!/usr/bin/env python3
"""
PTTechAI Benchmark Runner & Coverage Analyzer

Real execution of 104 CTF-style validation benchmarks:
  - Builds and starts each Docker challenge
  - Runs PTTechAI autonomous agent against the target
...
class ReportGenerator:
    ...
    def print_summary(results: List[BenchmarkResult]) -> None:
        ...
        print()
        print("=" * 70)
        print("  PTTechAI Benchmark Results")
        print("=" * 70)
        print()
...
class BenchmarkRunner:
    ...
    async def run_all(self) -> List[BenchmarkResult]:
        ...
        print(f"\n{'='*70}")
        print(f"  PTTechAI Benchmark Runner")
    ...
    async def _run_agent(self, target_url: str, agent_logs: List[str]) -> Tuple[Dict, list]:
        """Run the PTTechAI autonomous agent against a target."""
...
def load_pttechai_types() -> Tuple[Set[str], Dict]:
    """Load PTTechAI's 100 vulnerability types from registry."""
    try:
...
def analyze_coverage(benchmarks: List[Dict], ns_types: Set[str]) -> Dict:
    """Analyze PTTechAI coverage of benchmarks."""
    tag_counter = Counter()
    ...
    for tag in tag_counter:
        ns_mapped = TAG_TO_PTTechAI.get(tag, [])
        if ns_mapped:
...
def print_coverage_report(analysis: Dict):
    ...
    print()
    print("=" * 70)
    print("  PTTechAI BENCHMARK COVERAGE ANALYSIS")
    print("=" * 70)
    print()

    print(f"  Total Benchmarks:        {analysis['total_benchmarks']}")
    print(f"  PTTechAI Vuln Types:  {analysis['ns_total_types']}")
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PTTechAI Benchmark Runner & Coverage Analyzer",
...
Examples:
  ...
  %(prog)s coverage --json                       Coverage output as JSON
    ...
    if args.command == "run":
        ...
        ns_types, ns_info = load_pttechai_types()
